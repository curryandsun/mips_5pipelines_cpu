...
class Pipelines extends Module() {
  ...
  */
  ...
  when(alu.io.overflow){
    ...
    //lh
  }.elsewhen(ID_EX_CMPctr === LS_H && ID_EX_RegWr && alu.io.ALUout(0)=/=0.U){
    cp0.io.IsExc := 1.U
    cp0.io.Status := 0x2.U
    cp0.io.ExcCode := 0x04.U(5.W)
    cp0.io.BadVAddr := alu.io.ALUout


    //lhu
  }.elsewhen(ID_EX_CMPctr === LS_Hu && ID_EX_RegWr && alu.io.ALUout(0)=/=0.U){
    cp0.io.IsExc := 1.U
    cp0.io.Status := 0x2.U
    cp0.io.ExcCode := 0x04.U(5.W)
    cp0.io.BadVAddr := alu.io.ALUout
    ...
    //lw
  }.elsewhen(ID_EX_CMPctr === CMP_X && ID_EX_RegSrc === Regsrc_Mem && ID_EX_RegWr && alu.io.ALUout(1,0)=/=0.U){
    cp0.io.IsExc := 1.U
    cp0.io.Status := 0x2.U
    cp0.io.ExcCode := 0x04.U(5.W)
    cp0.io.BadVAddr := alu.io.ALUout
    ...
    //sh
  }.elsewhen(ID_EX_CMPctr === LS_H && ID_EX_MemWr && alu.io.ALUout(0)=/=0.U){
    cp0.io.IsExc := 1.U
    cp0.io.Status := 0x2.U
    ...
    //sw
  }.elsewhen(ID_EX_CMPctr === CMP_X && ID_EX_RegSrc === Regsrc_Mem && ID_EX_MemWr && alu.io.ALUout(1,0)=/=0.U){
    cp0.io.IsExc := 1.U
    cp0.io.Status := 0x2.U
    cp0.io.ExcCode := 0x05.U(5.W)
    cp0.io.BadVAddr := alu.io.ALUout
  ...
  */
  ...
  /*
  printf("Reg_index=%d,Reg_WrData=0x%x,RegWr?=%d\n",
    Reg_index,
    Reg_WrData,
    MEM_WB_RegWr
  )
